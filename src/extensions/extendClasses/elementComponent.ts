...
export class ElementComponent_EX extends pc.ElementComponent
{
    ...
    get calculatedCorners(): [pc.Vec3, pc.Vec3, pc.Vec3, pc.Vec3]
    {
        ...
        // 转换世界坐标为屏幕坐标
        const deviceWidth = pc.app.graphicsDevice.canvas.clientWidth;
        const deviceHeight = pc.app.graphicsDevice.canvas.clientHeight;
        for (let i = 0; i < 4; i++) {
    ...
    getPixelAt(screenPoint: { x: number, y: number }, camera?: pc.CameraComponent, color?: pc.Color): pc.Color
    {
        ...
        const currentCamera = camera ?? pc.app.context.systems.camera.cameras[0];
    ...
    setPosFromWorld(pos: pc.Vec3, camera?: pc.CameraComponent): void
    {
        ...
        const devicePixelRatio = pc.app.graphicsDevice.maxPixelRatio;
        ...
        // 设置该element的位置
        this.entity.setLocalPosition(_screenPosition.x / scale, (pc.app.graphicsDevice.height - _screenPosition.y) / scale, 0);
    }
}